...
def extract_product(products):
    """
    Extract a product from the list of products.
    """
    product = products[0]
...
def test_get_products(client: FlaskClient) -> None:
    ...
    if products:
        extract_product(products)


def test_get_nonexisting_products(client: FlaskClient) -> None:
    """
    Test the GET /products endpoint for nonexisting products (empty table in the db).
    """
    # Simulate a GET request to /products endpoint
    response = client.get("/products")

    # Ensure the request is failing (status code 404)
    ...
    assert error_message.get("error") == "Products not found"
...
def test_get_single_product(client: FlaskClient) -> None:
    ...
    if product:
        ...
        assert product.get("price_per_unit") == 1200.0
        assert product.get("uom_id") == 2


def test_get_nonexisting_product(client: FlaskClient) -> None:
    """
    ...
    # Ensure the value of the "error" field is the same
    # as defined in the routes implementation
    assert error_message.get("error") == "Product not found"
